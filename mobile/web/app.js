...
function addMessage(role,text,meta={}){const welcome=$('welcome');if(welcome)welcome.style.display='none';const el=document.createElement('div');el.className=`message ${role}`;const sources=Array.isArray(meta.sources)?meta.sources:[];const feedback=role==='assistant'?`<div class="mobile-feedback"><button data-rate="1">Útil</button><button data-rate="0">Não útil</button></div>`:'';el.innerHTML=`<div class="avatar">${role==='user'?'V':'✦'}</div><div><div class="message-head">${role==='user'?'Você':'Jarvis'}${meta.model?`<small>${esc(meta.model)}</small>`:''}</div><div class="message-body">${role==='assistant'?md(text):esc(text).replace(/\n/g,'<br>')}</div>${sources.length?`<div class="message-sources">${sources.slice(0,4).map(s=>s.url?`<a href="${esc(s.url)}" target="_blank">${esc(s.title||s.name||'Fonte')}</a>`:'').join('')}</div>`:''}${feedback}</div>`;el.querySelectorAll('[data-rate]').forEach(b=>b.onclick=async()=>{const positive=b.dataset.rate==='1';const d=await api('/api/experience/rate',{method:'POST',body:JSON.stringify({positive})});if(d.ok){b.textContent=positive?'Aprendido ✓':'Registrado ✓';b.disabled=true;await refresh()}});$('chat').appendChild(el);$('chat').scrollTop=$('chat').scrollHeight}
...
function renderSnapshot(){const p=snapshot.project;const conv=snapshot.conversations?.items||[];$('projectCard').innerHTML=p?`<b>${esc(p.name)}</b><span>${esc(p.description||'Projeto atual')}</span>`:'<b>Sem projeto ativo</b><span>Conversa geral</span>';$('metricGrid').innerHTML=`<div class="metric"><small>CONVERSAS</small><b>${conv.length}</b></div><div class="metric"><small>PLAYBOOKS</small><b>${snapshot.workplace?.playbooks||0}</b></div><div class="metric"><small>EXPERIÊNCIA</small><b>${snapshot.experience?.competences||0}</b></div><div class="metric"><small>JOBS</small><b>${snapshot.long_horizon?.active||0}</b></div><div class="metric"><small>PERFIL</small><b>${esc(snapshot.hardware?.tier||'local')}</b></div><div class="metric"><small>FAST</small><b>${esc((snapshot.model?.fast||'—').replace('qwen3:',''))}</b></div><div class="metric"><small>REASON</small><b>${esc((snapshot.model?.reason||'—').replace('qwen3:',''))}</b></div>`;$('conversationList').innerHTML=conv.map(c=>`<div class="conversation-row ${Number(c.id)===Number(snapshot.current_session)?'active':''}"><button class="conversation-item" data-session="${c.id}">${esc(c.title||'Conversa')}</button><button class="conversation-delete" data-delete-session="${c.id}" data-delete-title="${esc(c.title||'Conversa')}" aria-label="Excluir conversa">×</button></div>`).join('');document.querySelectorAll('[data-session]').forEach(b=>b.onclick=()=>selectConversation(Number(b.dataset.session)));document.querySelectorAll('[data-delete-session]').forEach(b=>b.onclick=e=>{e.stopPropagation();deleteConversation(Number(b.dataset.deleteSession),b.dataset.deleteTitle||'esta conversa')})}
...
async function deleteConversation(id,title){
  ...
  if(!ok)return;
  const d=await api('/api/conversation/delete',{method:'POST',body:JSON.stringify({session_id:id})});
  if(!d.ok){alert(d.error||'Não foi possível excluir a conversa.');return}
  await refresh();
  if(d.deleted_was_current)await loadHistory(d.session_id);
}
